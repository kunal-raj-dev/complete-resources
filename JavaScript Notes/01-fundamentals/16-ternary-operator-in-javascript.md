...
- **Must Remember:** Ternary is an expression that returns a value: `condition ? ifTrue : ifFalse`.
- **Most Common Confusion:** Trying to omit the `:` colon branch (ternaries require all 3 parts).
- **One Code Pattern:** Inline template string: `` `User is ${isOnline ? 'Online' : 'Offline'}` ``.
- **One Interview Question:** *"Why does React JSX allow ternary operators inside `{}` but disallows `if-else`?"*  
  $\to$ Because `{}` in JSX expects an expression that produces a value; `if-else` is a statement that returns nothing.
