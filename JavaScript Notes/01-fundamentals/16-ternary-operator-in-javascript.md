...
- The ternary operator (`? :`) takes three operands and returns a concrete value.
- Because it is an expression, it can be assigned directly to variables or passed to functions.
- Short-circuiting guarantees only the chosen branch expression is executed.
- Essential for JSX and template literals where statements like `if` are prohibited.
- Do not chain more than two ternaries; prefer `if...else if` or switch/lookup tables for complex branching.
